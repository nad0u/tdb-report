...
Virtual machines created on the \texttt{compute1} node are called \texttt{vm1}\rp{vm1 looks like one machine, do you mean their names start with ``vm1''?}, and those created on the \texttt{compute2} node are called \texttt{vm2}. If a virtual machine has a block storage attached to it, \texttt{bs} will be attached to its name, becoming \texttt{vm1bs} for example. This convention is followed in the rest of this report.
