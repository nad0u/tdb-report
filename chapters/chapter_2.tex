...
For our experiment, it was sufficient to only use Nova to manage networking instead of Neutron. \rp{I think we should remove legacy networking and just write about usual networking.}
...
\subsection{Cinder}
\rp{You already described a block storage before quite in details, please merge it here.}
...
However, Horizon cannot completely replace the command line tool.\rp{why? you need to support your claim if you write about something.}
...
\section{Setup}
\label{section_setup}
For our experiments, OpenStack Icehouse \rp{this is the first time Icehouse appears in the text, you need to specify what is it.} was installed and configured on three physical machines:
...
As installing OpenStack is a relatively long process, all the steps are not described in this section. The OpenStack documentation \cite{osinstall}
describes a well step-by-step that was followed in order to make everything work correctly.


\paragraph{Naming convention}\mbox{}\\
Virtual machines created on \texttt{compute1} are called \texttt{vm1}, and those created on \texttt{compute2} are called \texttt{vm2}. Moreover if the virtual machine has a block storage attached to it, \texttt{bs} will be attached to its name, becoming \texttt{vm1bs} for example. This convention will be followed for the rest of this report.


%%--------------- subsubsection
\subsection{Machines}
This section will present the hardware used for the experiments. We have three physical machines on which OpenStack services are installed, and several virtual machines with different flavors will be created on the physical machines.
...
The controller node is the heart of our OpenStack architecture, and thus all the different services APIs (Application Programming Interface) are installed on it. 
